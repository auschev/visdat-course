...
# hier wird das notwendige Werkzeug für die Datenaufbereitung und Visualisierung 
# aus Bilbliothek importiert
...
# hier werden bei csv Dateien einzeln geladen und auf die entsprechenden Variablen zugewiesen
# accel_df und gyro_df
# Die Daten stammen aus dem Ordner imu-workshop/data/raw
accel_df = pd.read_csv('imu-workshop/data/raw/auschev_accelerometer.csv')
gyro_df = pd.read_csv('imu-workshop/data/raw/auschev_gyroscope.csv')


# Ich habe fuer die Messung phyphox verwednet: Time, X, Y, Z
# Die Bezeichnung der Achsen wurde angepasst
accel_df.rename(columns={'Time (s)': 'time', 'X (m/s^2)': 'accel_x', 'Y (m/s^2)': 'accel_y', 'Z (m/s^2)': 'accel_z'}, inplace=True)
gyro_df.rename(columns={'Time (s)': 'time', 'X (rad/s)': 'gyro_x', 'Y (rad/s)': 'gyro_y', 'Z (rad/s)': 'gyro_z'}, inplace=True)

# Merge on timestamp (or use nearest time if sampling rates differ slightly)
# pd.merge_asof() verbindet beide Daten zeitbasiert, 
# indem es für jede Zeile des ersten DataFrames 
# die zeitlich nächste Zeile aus dem zweiten DataFrame sucht.
...
# bei Zeit gleich Null beginnen
df['time'] = df['time'] - df['time'].iloc[0]

####################################################################

# Calculate sampling rate (use median for robustness against jitter)
# berechnet die Differenz zwischen aufeinanderfolgenden Zeitstempeln 
# und nimmt den Median dieser Differenzen
dt = df['time'].diff().median()
# Berechnet die Abtastrate in Hertz (Hz) durch die Inversion der Zeitdifferenz
# Zeitabstand zwischen zwei Messpunkten
sampling_rate = 1 / dt
...
# Der Code prüft, ob die Gyroskopdaten in Grad pro Sekunde (deg/s) oder in Radiant pro Sekunde (rad/s)
# vorliegen – und konvertiert sie falls erforderlich sein sollte.
# deshalb wichtig, weil Madgwick-Algorithmus rad/s erwartet.

# diese drei Spalten enthalten die Gyroskopdaten
gyro_cols = ['gyro_x', 'gyro_y', 'gyro_z']

# hier wird bisschen Statistik genutzt um den Maximalwert der Gyroskopdaten zu bestimmen
max_gyro_value = df[gyro_cols].abs().quantile(0.95).max()


# Wenn der Maximalwert >20 ist, sind die Daten fast sicher in deg/s, 
# weil rad/s Werte normalerweise kleiner als 20 oft nur bis 10 rad/s liegen.
...
# Der Code zeichnet zwei Diagramme untereinander
# eins für den Beschleunigungssensor (Accelerometer) und eins für den Gyroskop-Sensor – jeweils über der Zeit.
...
# erstellt die Filterkoeffizienten für einen Butterworth-Filter
# und wendet diesen Filter auf die Daten an.
from scipy.signal import butter, filtfilt


# data ist das SIgnal
# cutoff ist die Grenzfrequenz
# fs ist die Abtastrate
# order ist die Ordnung des Filters
def butter_lowpass_filter(data, cutoff, fs, order=2):
    """Apply a Butterworth low-pass filter to the data."""
    nyquist = 0.5 * fs # Nyquist-Frequenz: halbe Abtastrate
    normal_cutoff = cutoff / nyquist
    b, a = butter(order, normal_cutoff, btype='low', analog=False)
    filtered_data = filtfilt(b, a, data)
    return filtered_data # gibt das gefilterte Signal zurück

# Filter parameters einstellen
# für eine lineare Beschleuningng entlang eines Tisches es sind 
# keine theoretisch keine plötzlichen Richtungsänderungen enthalten
# jedoch kann das handy nicht ruhig in der Hand gehalten werden
# daher können hochfrequente Anteile vorhanden sein, die das Ergebnis beeinflussen
# daher müssen diese rausgefiltert werden
# deshalb wird der Grenzfrequenz auf 5 Hz gesetzt
...
# Der Code erzeugt drei Diagramme untereinander, 
# die jeweils die rohen und die gefilterten Beschleunigungsdaten vergleichen
...
# aus den gefilterten Daten wird fuer jedes Sample die Orientierung des Handys berechnet
# dazu wird der Madgwick-Algorithmus verwendet
# der Madgwick-Algorithmus kombiniert die Daten von Beschleunigungssensor und Gyroskop
# um die Orientierung in Form von Quaternionen zu schätzen

from ahrs.filters import Madgwick

# Initialize the Madgwick filter
# gain: Gewichtung fuer die Beschleunigungsdaten und Gyroskopdaten
# höherer gain bedeutet mehr Gewichtung auf Beschleunigungsdaten
# niedrigerer gain bedeutet mehr Gewichtung auf Gyroskopdaten
...
# hier sind die jeweiligen euler Winkel gespeichert
# Roll, Pitch, Yaw
...
# hier werden die jeweiligen Achsen definiert, beschriftet 
...
# hier wird ein Vektor für die globalen Beschleunigungen erstellt
accel_global = np.zeros((len(df), 3))

# hier wird der relative Vektor in den absoluten Vektor umgerechnet 
# mithilfe von Transformationsmatrix oder Quaternionen
# Iteriert durch alle Zeitpunkte
...
#### hier habe ich die Achsen skaliert, damit die Trajektorie besser sichtbar ist
#### da die Bewegung hauptsächlich in X-Richtung stattfand

plt.figure(figsize=(10, 10))
plt.plot(df['pos_x'], df['pos_y'] * 0, linewidth=2, label='Trajectory')
plt.scatter(df['pos_x'].iloc[0], df['pos_y'].iloc[0], 
            c='green', s=200, marker='o', label='Start', zorder=5)
plt.scatter(df['pos_x'].iloc[-1], df['pos_y'].iloc[-1] * 0, 
...
# hier habe  ich die 3D Darstellung skaliert, damit die Trajektorie besser sichtbar ist
# da die Bewegung hauptsächlich in X-Richtung stattfand
...
# der Betrag des Vektors vom Start- zum Endpunkt
# hier habe ich den Betrag nicht skaliert, da ich die reale Distanz berechnen möchte
...
# tatsächlich mit der Hand abgefahrene Distanz ist etwa 0.6 Meter
# somit ist die Abweichung berechenbar und ergibt eine Differenz von 10 cm
actual_distance = 0.6  # meters (your measurement)
...
# 1) Positionen in DataFrame übernehmen
...
# 2) Sampling-Frequenz angeben (WICHTIG!)

fs = 100  # Beispiel: 100 Hz Abtastrate 
dt = 1.0 / fs


# 3) FFT-Funktion für eine Spalte
...
# 4) FFT für jede Achse berechnen

freq_x, fft_x = compute_fft(df['pos_x'], fs)
freq_y, fft_y = compute_fft(df['pos_y'], fs)
freq_z, fft_z = compute_fft(df['pos_z'], fs)
